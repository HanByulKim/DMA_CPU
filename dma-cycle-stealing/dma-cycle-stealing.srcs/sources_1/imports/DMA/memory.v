...
module Memory(clk, reset_n, i_readM, i_writeM, i_address, i_data, d_readM, d_writeM, d_address, d_data);
    ...
    reg [2:0] ImemCount; // count the cycle it took
    reg [2:0] DmemCount; // count the cycle it took
	...
	always@(negedge clk) begin
        if(reset_n) begin
            if(i_readM) begin
                if(ImemCount[2:0]==`MEMORY_DELAY-1) begin
                    ImemCount=0;
                end
                else begin 
            ...
            if(d_readM || d_writeM!=2'b00) begin
                if(DmemCount[2:0]==`MEMORY_DELAY-1) begin
                    DmemCount=0;
                end
                else begin 
                    DmemCount= DmemCount+1;
                end
            end            
        end
    end
		...
		if(!reset_n)
			...
			begin
                ...
                if(DmemCount[2:0]==`MEMORY_DELAY-1) begin
                    ...
                    if(d_writeM==2'b10) memory[d_address] <= d_data;
                    else if(d_writeM==2'b01) begin
                        memory[d_address+3] <= d_data[63:48];
                        memory[d_address+2] <= d_data[47:32];
                        memory[d_address+1] <= d_data[31:16];
                        memory[d_address+0] <= d_data[15:0];
                    end                   
                end
			end
endmodule