...
always@(posedge DMABegin) begin     // dma begin-interrupt handling
    DMACMD=1;
end
...
always@(BR or cache_d_readM or cache_d_writeM) begin
    if(BR==1&&BG==0&&cache_d_readM==0&&cache_d_writeM==0) begin
        BG<=1;  // when BR is on and the cache is not accessing the memory, turn on BG signal
    end
end

always@(negedge BR) begin
    BG<=0;  //when DMA turn off BR signal, CPU take the bus again
end

always@(posedge clk) begin
    if(reset_n==1&&is_halted==0&&cacheWorking==0) begin     //if data cache miss occurs when CPU granted the bus to DMA, cacheWorking signal stalls CPU 
